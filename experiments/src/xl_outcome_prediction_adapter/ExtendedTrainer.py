...
class ExtendedTrainer:
    ...
    def _report_to_hp_search(
        ...
        tmp_metrics = dict([(k,v) for k,v in metrics.items() if k in ['eval_val_pr_auc']])
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            ####MALLON EDW PREPEI NA GINEI H ANTIKATASTASH GLWSSWN
            # LANGUAGE DURCHSCHLEIFEN!!!
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    #add_more_metrics changed
                    metrics = self._maybe_log_save_evaluate(tr_loss, model, trial, epoch)
    ...
    def compute_metrics_report(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        ...
        return classification_report(y_true=output.label_ids, y_pred=np.argmax(output.predictions, axis=1),
                                     output_dict=True)
    ...
    def evaluate(self, is_trained = None, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        Run evaluation and returns metrics.
        ...
        output = self.prediction_loop(
            ...
            # No point gathering the predictions if there are no metrics, otherwise we defer to
            # self.args.prediction_loss_only
            prediction_loss_only=False if self.compute_metrics is None else None,
        )
        ...
        #    self.log(output.metrics)
        #    tune.report(
        #        eval_val_loss=output.metrics['eval_val_loss'],
        #        eval_val_auc=output.metrics['eval_val_auc']
        #       )
